...
def live_view(self, link, xpaths, base_path, diff_path, url, keywords_diff, detect_links,\
    ...
    if remote_content is not None and local_content is not None:
        ...
        # print('len (remote_content) before = {}'.format(len(remote_content)))
        # with open('frd.html', 'w+') as fd:
            # print('writing ..............')
            # fd.write(remote_content)
        # print('STR REMOTE CONTENT : {}'.format(str(remote_content)))
        
        # remote_content = remote_content.replace('&eacute', '&#233;').replace('&egrave', '&#232;').replace('&euro', '&#8364;')
        targeted_content = etree.HTML(remote_content.encode('utf-8'))
        # print('DOM : {}'.format(targeted_content.xpath(xpaths[0])))
        print('targeted_content : {}'.format(targeted_content))
        # targeted_content = remote_content
        # for xp in xpaths:
            # targeted_content = targeted_content.xpath(xp)
        # print('xpaths = {}'.format(xpaths))
        if len(xpaths) == 1:

            print('XPATH 0 ===> {}'.format(xpaths[0].lower()))

            # if 'time' in xpaths[0].lower():
                # print('time found : {}'.format(etree.fromstring(remote_content)))
            remote_content = remote_content.replace('&eacute', '&#233;').replace('&egrave', '&#232;').replace('&euro', '&#8364;')
            doc = LH.fromstring(remote_content)#.replace('\'', ' '))
# try:
            # targeted_content = doc.xpath('{}'.format(xpaths[0].lower()))
            print('xpaths = {}'.format(xpaths[0].lower()))
            # targeted_content = targeted_content.xpath('{}'.format(xpaths[0].lower()))
            targeted_content = targeted_content.xpath('/html/body/div[8]/div[3]')
            print('other xpath -> {}'.format(targeted_content))
            # targeted_content = targeted_content.xpath('/html/body/div[8]/div[3]/div')
            remote_content = targeted_content
            # targeted_content = targeted_content.xpath('{s!r}'.format(s = xpaths[0]))#("//table[tr/td/font/a/b='Rank']//text()")
            print('targeted_content after xpath selection  => {}'.format(targeted_content))

        elif len(xpaths) > 1:
            print('\n\n/!\\ XPath greater than one. Still testing.')

        print('len (remote_content) after = {}'.format(len(targeted_content)))
        ...
        if detect_links:
            ...
            for _ in status['all_links_pos']:
                ...
                if isinstance(res, bool) and res is True:
                    print('[FastText] SBB file detected ({})'.format(_))
