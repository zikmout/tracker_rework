			...
			if 'fromPage' not in args:
				...
				if args['fromPage'] == 'live_view' and 'live_view' in self.session['tasks']:
					...
					if len(task_results) == 1:
						html += " website has changed (" + args['mailAlertType'] + " alert): </a><br> "
					else:
						html += " websites have changed (" + args['mailAlertType'] + " alert): </a><br> "
					...
					if errors != [] or errs != {}:
						html += "<br><br><b>Errors : (" + str(len(errors) + len(errs)) + "/" + str(total_scanned) + " fetched)</b><br>"
						if errors != []:
							for err in errors:
					...
					for email in receiver_email:
						...
						message["Subject"] = '[Live Alert] {}'.format(', '.join(list(set(domains_list))))
