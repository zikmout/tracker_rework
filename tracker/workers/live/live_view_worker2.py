...
from tracker.celery import live_view_worker_app2
...
@live_view_worker_app2.task(bind=True, ignore_result=False, soft_time_limit=50)#, time_limit=5)
def live_view2(self, link, base_path, diff_path, url, keywords_diff, detect_links,\
...
# @app.task(bind=True)
# def log_error_live_view(self, z):
#     print('ERROR for live_view_task = {}'.format(z))

@live_view_worker_app2.task(bind=True)
def live_view_end_routine(self, task_results):#, mails, user_email, project_name, show_links):#, soft_time_limit=120):
    # print('Here in live_view_end_routine, TASK RESULT= {}'.format(task_results))
    errors = dict()
